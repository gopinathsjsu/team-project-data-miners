...
def cancel_Booking(id):
    ...
    '''
    with engine.connect() as con:
      query = '''UPDATE `Booking.Booking_details` SET status = 'Cancelled' where Booking_id = {id}'''.format(id=id)
      con.execute(query)
...
def complete_booking(first_name,last_name,email_id,phone,amenities,roomtype,check_in,checkout,meal,guest,hotelid,user_id):
  ...
  with engine.connect() as con:
      query='''
      insert into `Booking.Booking_details`(first_name,last_name,email_id,phone,Customer_id,Booking_date,Check_in_date,check_out_date,No_of_guest,hotel_id,room_type_id,Amenities,meal
) values('{first_name}','{last_name}','{email_id}',{phone},{user_id},'{current_date}','{check_in}','{checkout}',{guest},{hotelid},{roomtype_id},'{amenities}','{meal}')'''.format(first_name=first_name,last_name=last_name,email_id=email_id,phone=phone,user_id=user_id,current_date=current_date,check_in=check_in,checkout=checkout,guest=guest,hotelid=hotelid,roomtype_id=roomtype_id,amenities=amenities,meal=meal)
      try:
        rs = con.execute(query)
      except:
        return False
...
def get_all_booking():
    ...
    """
    with engine.connect() as con:
        query='''SELECT bd.Booking_id, cd.Customer_first_name,cd.Customer_last_name,cd.Customer_Phone,cd.Customer_email_id, bd.Check_in_date,bd.check_out_date,
...
def get_customer_booking_details(id):
    """
    :return:
    """
    with engine.connect() as con:
        query='''SELECT bd.Booking_id, bd.first_name, bd.last_name, bd.phone, bd.email_id, bd.Check_in_date, bd.check_out_date, bd.No_of_guest, h.Hotel_name, hr.Room_type_Name, bd.Amenities, bd.meal, bd.status FROM `Booking.Booking_details` bd inner join `Hotel.Hotels` h on h.Hotel_id=bd.hotel_id inner join `Hotel.Rooms_types` hr on hr.Room_type_id=bd.room_type_id WHERE Customer_id = {id};'''.format(id = id)
        rs = con.execute(query)
        final=[]
        for row in rs:
            final.append(row)
...
def get_rewards(id):
  with engine.connect() as con:
    query = '''SELECT Points from `Rewards.Customer_Points` where  Customer_id = {id}'''.format(id = id)
    points = con.execute(query)
    for x in points:
      return x
